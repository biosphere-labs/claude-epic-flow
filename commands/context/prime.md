...
- If directory doesn't exist or is empty:
  ...
  - Exit gracefully
- Count available context files: `ls -1 workflow/context/*.md 2>/dev/null | wc -l`
- Report: "📁 Found {count} context files to load"

### 2. File Integrity Check
- For each context file found:
  - Verify file is readable: `test -r "workflow/context/{file}" && echo "readable"`
  - Check file has content: `test -s "workflow/context/{file}" && echo "has content"`
