...
Before decomposing into tasks, validate architectural boundaries:

```yaml
Task:
  subagent_type: "general-purpose"
  prompt: |
    As an architecture strategist, analyze this epic for task decomposition:

    {epic content from workflow/epics/$ARGUMENTS/epic.md}

    Verify and recommend:
    1. Component boundaries are properly defined for task separation
    2. No layer violations in the proposed approach
    3. Changes align with existing architecture patterns
    4. Dependencies are properly scoped between components

    Return:
    - Architecture concerns (if any) that should be addressed before decomposition
    - Recommended task groupings based on component boundaries:
      - Frontend tasks (components, pages, state)
      - Backend tasks (services, endpoints, guards)
      - Data tasks (models, migrations, seeds)
      - Integration tasks (cross-cutting concerns)
    - Suggested parallel execution opportunities
    - Files/areas that multiple tasks will touch (potential conflicts)
```

Use this analysis to inform task creation and groupings in subsequent steps.
...
Task:
  ...
  prompt: |
    ...
    For each task:
    1. Create file: workflow/epics/$ARGUMENTS/{number}.md
