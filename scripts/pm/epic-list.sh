...
if [ ! -d ".claude/epics" ]; then
  echo "📁 No epics directory found. Create your first epic with: /pm:epic-create <feature-name>"
  exit 0
fi
epic_dirs=$(ls -d workflow/epics/*/ 2>/dev/null || true)
if [ -z "$epic_dirs" ]; then
  echo "📁 No epics found. Create your first epic with: /pm:epic-create <feature-name>"
  exit 0
fi
...
echo ""
echo "📊 Summary"
total=$(ls -d workflow/epics/*/ 2>/dev/null | wc -l)
